...
// ── Seed EPG providers (only 2 sources) ─────────────────────────────
// Clean up old providers and ensure only the 2 active sources exist
db.prepare("DELETE FROM epg_providers WHERE url NOT IN ('https://opop.pro/3rBcgkD4sZQ4', 'https://raw.githubusercontent.com/dp247/Freeview-EPG/master/epg.xml')").run();

const providerCount = db.prepare('SELECT COUNT(*) as cnt FROM epg_providers').get();
if (providerCount.cnt === 0) {
  const seedProviders = [
    { name: 'opop.pro (Primary)', country: 'ALL', url: 'https://opop.pro/3rBcgkD4sZQ4', format: 'xmltv', type: 'direct', channels: 0, notes: 'Primary EPG source. Arabic, sports, entertainment, news channels.' },
    { name: 'Freeview-EPG (Fallback)', country: 'GB', url: 'https://raw.githubusercontent.com/dp247/Freeview-EPG/master/epg.xml', format: 'xmltv', type: 'direct', channels: 200, notes: 'UK Freeview EPG fallback. 7 days data, updated every 12h. GitHub: dp247/Freeview-EPG' },
  ];
